...
def main():
    ...
    num_z = num_z.ravel()
    w = np.logspace(-6, np.log10(np.pi / dt), 400)
    _, mag_z, _ = signal.dbode((num_z, den_z, dt), w=w * dt)
    _, mag_s, _ = signal.bode((modelo_control.NUM_S, modelo_control.DEN_S), w=w)
    fig, ax = plt.subplots(figsize=(10, 4.5))
    ax.semilogx(w, mag_z - mag_z[0], color="#cf545a", lw=1.6, label="G(z) latente")
    ax.semilogx(w, mag_s - mag_s[0], color="#3a8fb7", lw=1.6, label="G(s)")
    ax.set_xlabel("w (rad/s)")
    ax.set_ylabel("|G| normalizada a DC (dB)")
    ax.set_title("Respuesta en frecuencia: heater -> temperatura")
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
